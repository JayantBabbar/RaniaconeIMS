...
//
// Server-aggregated. We render rows + totals straight out of
// the response — no front-end summing — so what you see on
// screen is what would print on a PDF / GSTR upload.
// ═══════════════════════════════════════════════════════════

function defaultStart(): string {
  // First of current month, in viewer-local calendar terms.
  const now = new Date();
...
export default function SalesRegisterPage() {
  ...
  const customers = useMemo(
    () => (partiesRaw?.data ?? []).filter((p) => p.party_type === "customer" || p.party_type === "both"),
    [partiesRaw],
  );

  const { data, isLoading } = useQuery({
    queryKey: ["report-sales", { startDate, endDate, partyFilter }],
    queryFn: () =>
      reportsService.salesRegister({
  ...
  const exportCsv = () => {
    ...
    const headers = ["Invoice #", "Date", "Party", "State", "Taxable", "CGST", "SGST", "IGST", "Cess", "Total", "Status"];
    const lines = [
      headers.join(","),
      ...rows.map((r) => [
    ...
    a.href = url;
    a.download = `sales-register_${startDate}_to_${endDate}.csv`;
    a.click();
      ...
      <div className="px-4 lg:px-6 py-4 max-w-7xl mx-auto">
        ...
        {isLoading ? (
          ...
          <EmptyState
            ...
            description="Try widening the date range or clearing the customer filter."
          />
        ) : (
          <div className="rounded-lg border border-border bg-bg-elevated overflow-x-auto">
            <table className="w-full text-[13px]">
              ...
              <tbody>
                {rows.map((r) => (
                  <tr key={r.invoice_id} className="border-t border-border hover:bg-bg-hover">
                    <td className="px-3 py-2 font-mono text-[12px]">
                      <Link href={`/billing/invoices/${r.invoice_id}`} className="text-brand hover:underline">{r.invoice_number}</Link>
                    </td>
                    <td className="px-3 py-2 text-text-tertiary">{formatDate(r.invoice_date)}</td>
