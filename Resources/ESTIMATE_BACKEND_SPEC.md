...
3. **Copy the lines with the 2× qty + halve-price + reverse-GST rule** — for each `estimate_lines` row, create a corresponding `invoice_lines` row:
   ...
   - `rate_pct` from `estimate_lines.rate_pct` if set, else from `items.default_tax_rate_pct`, else from `item_categories.gst_rate_pct`. If still null → `422 RATE_REQUIRED_FOR_INVOICE`.
