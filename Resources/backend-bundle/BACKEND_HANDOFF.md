...
**Spec:** [`PARTY_PRICING_BACKEND_SPEC.md`](PARTY_PRICING_BACKEND_SPEC.md) · **Estimate promote rule:** [`ESTIMATE_BACKEND_SPEC.md`](ESTIMATE_BACKEND_SPEC.md) §5.8 · **Status:** FE complete in demo · **Updated:** 2026-05-13
...
2. **GST rate on `item_categories`.** New column `gst_rate_pct NUMERIC(5,2) NOT NULL DEFAULT '18'`. New items inherit this into `items.default_tax_rate_pct` on create; admin can override per-item. Seed all existing Arpit-tenant categories with `'18'`.

3. **Per-party pricing — two new versioned tables.** `party_item_costs` (supplier→Arpit cost lists) and `party_item_sale_prices` (Arpit→customer sale prices, GST-inclusive). Same `valid_from`/`valid_until` versioning as `item_pricing_rules`. Lookup is a hot path on `/bills/new` (supplier costs) and `/invoices/new` + `/estimates/new` (customer prices). Fallback for missing customer rule: `Item.default_sale_price`; no fallback for supplier costs (admin enters manually).

4. **Estimate→Invoice doubling + reverse-GST.** When an Estimate is auto-promoted to an Invoice, line `quantity` is multiplied by `ESTIMATE_TO_INVOICE_QTY_MULTIPLIER` (= 2 for Arpit's tenant) and `unit_price` is the reverse-GST'd taxable base of the estimate's customer-visible unit price. Stock is NOT moved again — the existing `estimate_id`-skip-stock-OUT invariant on invoice-post is the load-bearing rule.
